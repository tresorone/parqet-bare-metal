...
pub fn get_xirr(amounts: Vec<f64>, timestamps: Vec<i64>) -> f64 {
    let dates: Vec<DateTime<Utc>> = timestamps.into_iter().map(|t| Utc.timestamp_millis_opt(t).unwrap()).collect();
    ...
    let year_diff = diff_in_years(interval_start, last_transaction_date).expect("Dates ordered descending!");
    ...
    let durations: Vec<f64> = dates.iter().enumerate().map(|(idx, datetime)| {
        if idx == 0 {
            return 0.0;
        }
    ...
    let mut rate: f64 = if first_guess.abs() > 1.0 { 0.0 } else { first_guess };
    let mut npv = f64::MAX;
